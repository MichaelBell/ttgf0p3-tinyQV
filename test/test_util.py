...
async def setup_flash(dut):
    ...
    assert dut.qspi_ram_a_select.value == 1
    assert dut.qspi_ram_b_select.value == 1
    ...
    for i in range(8):
        ...
        cmd <<= 1
        await ClockCycles(dut.clk, 1, False)
        assert dut.qspi_flash_select.value == (0 if i < 7 else 1)
        assert dut.qspi_ram_a_select.value == 1
        assert dut.qspi_ram_b_select.value == 1
        assert dut.qspi_clk_out.value == 0
...
async def setup_ram(dut, ram_a):
    assert dut.qspi_flash_select.value == 1
    assert dut.qspi_ram_a_select.value == (0 if ram_a else 1)
    assert dut.qspi_ram_b_select.value == (1 if ram_a else 0)
    assert dut.qspi_clk_out.value == 0
    assert dut.qspi_data_oe.value == 1
